...
const fetchData = (userid, name, dayz, callback) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    if (name !== '0' && dayz !== '0') {
      query =
      "SELECT u.user_id, u.username, u.user_name, (SELECT title FROM book WHERE isbn = r.isbn) AS title, r.title, r.school_id, DATEDIFF(NOW(), r.date_of_rent) - 7 AS days_passed " +
      "FROM users u " +
    ...
    //dayz
      ...
      query =
      "SELECT u.user_id, u.username, u.user_name, r.isbn,  (SELECT title FROM book WHERE isbn = r.isbn) AS title, r.school_id, DATEDIFF(NOW(), r.date_of_rent) - 7 AS days_passed " +
      "FROM users u " +
      "INNER JOIN rent r ON u.user_id = r.user_id " +
      "INNER JOIN handlers h ON h.school_id = r.school_id " +
      "WHERE h.user_id = " + userid +
